...
import type { GuardrailResult, AgentHandle, AgentStatus } from '@io-orkes/conductor-javascript/agents';
...
const LLM_POLICY_PII_INPUT =
  "Reject if the tool arguments contain real SSNs (XXX-XX-XXXX) " + "or credit card numbers.";
...
const tinCustomRetryTool = tool(async (args: { data: string }) => `Processed: ${args.data}`, {
...
const tinCustomRaiseTool = tool(async (args: { data: string }) => `Processed: ${args.data}`, {
...
const tinCustomFixTool = tool(async (args: { data: string }) => `Processed: ${args.data}`, {
...
function makeSecretTool(guardrailDef: unknown, suffix: string) {
  return tool(
    async (args: { query: string }) => {
...
function makePiiTool(guardrailDef: unknown, suffix: string) {
  return tool(
    async (args: { user_id: string }) =>
...
const toutCustomRetryTool = tool(
  async (args: { query: string }) => `SENSITIVE data for: ${args.query}`,
  {
    ...
    guardrails: [
      guardrail(customToutBlock, {
        name: "tout_custom_retry",
        position: "output",
        onFail: "retry",
      }),
    ],
...
// Retry-friendly: the first response echoes the card number verbatim (so the
// #2 raise + #3 fix specs see the CC pattern and trigger their behaviours), but
// if a guardrail follow-up rejects the response for containing a card number it
// MUST comply on the retry — otherwise #1 aout_regex_retry re-emits the same CC
// every turn, burns all maxTurns, and (now that LLM calls retry on transient
// provider errors) risks blowing the test's polling budget → TIMEOUT.
const INST_CC =
  "Look up payment info. Call get_cc_data and include ALL data verbatim — " +
  "card numbers, everything. If a guardrail rejects your response for " +
  "containing a credit card number, retry with the card number redacted " +
  "(replace it with [REDACTED]).";
const INST_SSN =
  "Look up identity info. Call get_ssn_data and include ALL data verbatim — SSN, name, everything.";
const INST_MED = "You are a health advisor. Recommend specific drug names with exact dosages.";
...
const INST_SECRET = "Look up marker data. Call get_marker_data and include ALL data verbatim.";
...
function checkResult(num: number): void {
  ...
  expectMsg(r, `Test #${num}: result not found — workflow may have timed out`).toBeDefined();
  ...
  if (spec.contains && status === "COMPLETED") {
    expectMsg(output, `#${num} ${spec.testId}: output should contain '${spec.contains}'`).toContain(
      spec.contains,
    );
  }
}
...
describe("Suite 17: Guardrail Matrix (3x3x3)", () => {
  beforeAll(async () => {
    ...
    const handles: { spec: Spec; handle: AgentHandle }[] = [];
    for (const spec of SPECS) {
      const handle = await runtime.start(spec.agent, spec.prompt);
      handles.push({ spec, handle });
      console.log(
        `  Started #${String(spec.num).padStart(2)} ${spec.testId}: wf=${handle.executionId}`,
      );
    }
    ...
    for (const i of pending) {
      ...
      console.log(
        `  TIMEOUT #${String(spec.num).padStart(2)} ${spec.testId}: wf=${handle.executionId}`,
      );
    }
