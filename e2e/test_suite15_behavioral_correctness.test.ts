...
import {
  ...
  getOutputText,
  runDiagnostic,
  findToolTasksDeep, expectMsg } from './helpers';


jest.setTimeout(1_800_000); // ported from vitest describe({ timeout }) options
jest.retryTimes(2, { logErrorsBeforeRetry: true }); // upstream: per-test { retry: 2 } for LLM variability; jest only supports file scope
...
describe('Suite 15: Behavioral Correctness', () => {
  beforeAll(async () => {
  ...
  describe('Handoff Behavioral', () => {
    ...
    it('test_order_query_returns_tool_data', async () => {
      ...
      expectMsg(result.status, `[Handoff/Order] ${diag}`).toBe('COMPLETED');
    ...
    it('test_inventory_query_returns_stock_data', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Handoff/Inventory] ${diag}`).toBe('COMPLETED');
    ...
    it('test_shipping_query_returns_rate_data', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Handoff/Shipping] ${diag}`).toBe('COMPLETED');
  ...
  describe('Sequential Behavioral', () => {
    it('test_three_stage_pipeline_builds_on_prior', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Sequential/Pipeline] ${diag}`).toBe('COMPLETED');
    ...
    it('test_translator_pipeline_transforms_content', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Sequential/Translator] ${diag}`).toBe('COMPLETED');
    });
  });
  ...
  describe('Parallel Behavioral', () => {
    ...
    it('test_three_analysts_all_contribute', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Parallel/Analysts] ${diag}`).toBe('COMPLETED');
      ...
      expectMsg(weatherTask, `[Parallel/Analysts] get_weather task not found. ${taskDiag}`).toBeTruthy();
      expectMsg(weatherTask.status, `[Parallel/Analysts] get_weather not COMPLETED`).toBe('COMPLETED');

      const invTask = tasks['check_inventory'];
      expectMsg(invTask, `[Parallel/Analysts] check_inventory task not found. ${taskDiag}`).toBeTruthy();
      expectMsg(invTask.status, `[Parallel/Analysts] check_inventory not COMPLETED`).toBe('COMPLETED');

      const shipTask = tasks['get_shipping_rate'];
      expectMsg(shipTask, `[Parallel/Analysts] get_shipping_rate task not found. ${taskDiag}`).toBeTruthy();
      expectMsg(shipTask.status, `[Parallel/Analysts] get_shipping_rate not COMPLETED`).toBe('COMPLETED');
    });
    ...
    it('test_parallel_agents_produce_distinct_content', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Parallel/Distinct] ${diag}`).toBe('COMPLETED');
  ...
  describe('Router Behavioral', () => {
    ...
    it('test_weather_routed_and_tool_used', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Router/Weather] ${diag}`).toBe('COMPLETED');
    ...
    it('test_math_routed_and_computed', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Router/Math] ${diag}`).toBe('COMPLETED');
    ...
    it('test_order_routed_and_looked_up', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Router/Order] ${diag}`).toBe('COMPLETED');
      ...
      expectMsg(orderTask, `[Router/Order] lookup_order task not found. ${taskDiag}`).toBeTruthy();
      expectMsg(orderTask.status, `[Router/Order] lookup_order not COMPLETED`).toBe('COMPLETED');
  ...
  describe('Round Robin Behavioral', () => {
    it('test_collaborative_story_building', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[RoundRobin/Story] ${diag}`).toBe('COMPLETED');
    ...
    it('test_round_robin_with_tools_alternating', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[RoundRobin/Tools] ${diag}`).toBe('COMPLETED');
  ...
  describe('Multi-Topic Handoff', () => {
    ...
    it('test_dual_topic_order_and_shipping', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[MultiTopic/Dual] ${diag}`).toBe('COMPLETED');
    ...
    it('test_single_topic_stays_focused', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[MultiTopic/SingleFocus] ${diag}`).toBe('COMPLETED');
    ...
    it('test_all_three_via_sequential', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[MultiTopic/Sequential] ${diag}`).toBe('COMPLETED');
      ...
      const orderTask = tasks['lookup_order'];
      expectMsg(orderTask, `[Sequential] lookup_order task not found. ${taskDiag}`).toBeTruthy();
      expectMsg(orderTask.status, `[Sequential] lookup_order not COMPLETED`).toBe('COMPLETED');
      expectMsg(
        JSON.stringify(orderTask.output),
      ...
      expectMsg(inventoryTask, `[Sequential] check_inventory task not found. ${taskDiag}`).toBeTruthy();
      expectMsg(inventoryTask.status, `[Sequential] check_inventory not COMPLETED`).toBe('COMPLETED');
      expectMsg(
      ...
      expectMsg(shippingTask, `[Sequential] get_shipping_rate task not found. ${taskDiag}`).toBeTruthy();
      expectMsg(shippingTask.status, `[Sequential] get_shipping_rate not COMPLETED`).toBe('COMPLETED');
      expectMsg(
    ...
    it('test_parallel_all_specialists_contribute', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[MultiTopic/Parallel] ${diag}`).toBe('COMPLETED');
  ...
  describe('Cross-Strategy', () => {
    it('test_parallel_with_tool_agents', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Cross/Parallel] ${diag}`).toBe('COMPLETED');
    ...
    it('test_sequential_where_later_needs_earlier_data', async () => {
      ...
      const diag = runDiagnostic(result as unknown as Record<string, unknown>);
      expectMsg(result.status, `[Cross/Sequential] ${diag}`).toBe('COMPLETED');
