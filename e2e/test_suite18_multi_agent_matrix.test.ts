...
import {
  checkServerHealth,
  MODEL,
  getOutputText, expectMsg } from './helpers';
...
const analyzeData = tool(
  async (_args: { data_summary: string }) => ({
...
function checkResult(results: Map<number, TestResult>, num: number) {
  ...
  expectMsg(r, `Test ${num}: no result found`).toBeDefined();
  expectMsg(
    r.status,
...
describe('Suite 18: Multi-Agent Matrix', () => {
  ...
  beforeAll(async () => {
    ...
    expectMsg(specs.length, 'Expected exactly 21 test specs').toBe(21);
