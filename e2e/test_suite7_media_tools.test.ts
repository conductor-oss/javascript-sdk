...
import { Agent, AgentRuntime, imageTool, audioTool } from '@io-orkes/conductor-javascript/agents';
import {
  checkServerHealth,
  MODEL,
  TIMEOUT,
...
async function assertMediaGenerated(
  ...
  expectMsg(result.status, `[${stepName}] ${diag}`).toBe('COMPLETED');

  const task = await findMediaTask(result.executionId, taskTypePrefix);
  expectMsg(task, `[${stepName}] No ${taskTypePrefix} task in workflow`).toBeDefined();

  const taskStatus = String(task!.status ?? '');

  expectMsg(taskStatus, `[${stepName}] ${taskTypePrefix} task status`).toBe('COMPLETED');
  expectMsg(task!.outputData, `[${stepName}] empty outputData`).toBeDefined();
}
...
function assertToolCompiled(
  ...
  expectMsg(matching.length, `[${stepName}] No ${expectedToolType} tool`).toBeGreaterThanOrEqual(1);
  const config = matching[0].config as Record<string, unknown> | undefined;
  const model = config?.model ?? matching[0].model;
  expectMsg(model, `[${stepName}] wrong model`).toBe(expectedModel);
}

// ── Tests ───────────────────────────────────────────────────────────────

describe('Suite 7: Media Tools', () => {
  (!process.env.OPENAI_API_KEY ? it.skip : it.failing)('image — OpenAI DALL-E 3', async () => {
    const img = imageTool({
      name: 'gen_image',
  ...
  itSkipIf(!process.env.GOOGLE_AI_API_KEY)('image — Gemini Imagen 3', async () => {
    const img = imageTool({
  ...
  itSkipIf(!process.env.OPENAI_API_KEY)('audio — OpenAI TTS-1', async () => {
    const aud = audioTool({
      name: 'gen_audio',
