...
export class DefaultLogger implements ConductorLogger {
  ...
  constructor(config: DefaultLoggerConfig = {}) {
    ...
    this.tags = tags
    // AGENTSPAN_LOG_LEVEL is the deprecated spelling of
    // CONDUCTOR_AGENT_LOG_LEVEL. Unlike the other renamed vars this one falls
    // back silently: warning here would mean logging through the very logger
    // whose level is still being resolved.
    const resolvedLevel =
      level ??
      (process.env.CONDUCTOR_LOG_LEVEL as ConductorLogLevel | undefined) ??
      (process.env.CONDUCTOR_AGENT_LOG_LEVEL as ConductorLogLevel | undefined) ??
