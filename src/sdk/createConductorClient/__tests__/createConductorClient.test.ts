...
describe("createConductorClient integration", () => {
  ...
  describe("config resolution", () => {
    ...
    it("falls back to CONDUCTOR_AGENT_SERVER_URL when CONDUCTOR_SERVER_URL and explicit config are absent (spec R3)", async () => {
      process.env.CONDUCTOR_AGENT_SERVER_URL = "http://conductor-fallback:9090";
      const client = await createConductorClient({}, async () => jsonResponse({}));
      expect(client.getConfig().baseUrl).toBe("http://conductor-fallback:9090");
      delete process.env.CONDUCTOR_AGENT_SERVER_URL;
    });

    it("explicit config wins over CONDUCTOR_AGENT_SERVER_URL", async () => {
      process.env.CONDUCTOR_AGENT_SERVER_URL = "http://conductor-fallback:9090";
      const client = await createConductorClient(
