...
#   - Conductor server → CONDUCTOR_AGENT_SERVER_URL (default http://localhost:8080/api)
...
#   - CONDUCTOR_AGENT_CLI_PATH (default `agentspan` on PATH) — CLI suites skip if absent.
#
...
|-----------------------------------|------------------------|-----------------------------|
| node >= 20                        | —                      | —                           |
| Conductor server w/ agent runtime | `CONDUCTOR_AGENT_SERVER_URL` | `http://localhost:8080/api` |
| LLM model                         | `CONDUCTOR_AGENT_LLM_MODEL`  | `openai/gpt-4o-mini`        |
| mcp-testkit (MCP suites)          | `MCP_TESTKIT_URL`      | `http://localhost:3001`     |
| Conductor CLI (CLI suites)        | `CONDUCTOR_AGENT_CLI_PATH`   | `agentspan` (on `PATH`)     |
